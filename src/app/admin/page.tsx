...
export default async function AdminOverviewPage() {
  ...
  const [
    ...
    nannies,
    banned,
    publishedNannyProfiles,
    publishedParentProfiles,
    pendingReports,
  ] = await Promise.all([
    ...
    supabase.from('parent_profiles').select('id', { count: 'exact', head: true }).eq('is_published', true),
  ...
  const stats = [
    ...
    {
      label: 'Opublikowane profile',
      value: (publishedNannyProfiles.count ?? 0) + (publishedParentProfiles.count ?? 0),
    },
    { label: 'Zbanowani', value: banned.count ?? 0 },
    { label: 'Oczekujące zgłoszenia', value: pendingReports.count ?? 0 },
  ]
